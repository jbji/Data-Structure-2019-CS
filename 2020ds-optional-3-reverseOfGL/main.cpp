...
GLNode *reverse(GLNode *g) {
    ...
    if(g->tag == LIST){
        ...
        if(g->tp){
            node * toaddtp;
            if(g->tp->tp == nullptr){
                result->ptr.hp = reverse(g->tp->ptr.hp);
                toaddtp = result;
            }else{
                delete result;
                result = reverse(g->tp);
                //find the end, and attach the head;
                node * end = result->tp;
                do{
                    toaddtp = end;
                    end = end->tp;
            ...
            toaddtp->tp = new node();
            toaddtp->tp->tag = LIST;
            toaddtp->tp->ptr.hp = reverse(g->ptr.hp);
            toaddtp->tp->tp = nullptr;
        }else{
            result->ptr.hp = reverse(g->ptr.hp);
            result->tp = nullptr;
        }
    }
    return result;
