...
Status ListTraverse(SqList &L, void (*visit)(ElemType)) {   // 依次对L的每个元素调用函数visit()。若visit()失败，则操作失败
...
Status LocateElem(SqList L, ElemType e,
                  Status (*compare)(ElemType, ElemType)) {   //在L中查找与元素 e 满足compare() 的第 1 个元素
...
Status ListInsert(SqList &L, int i, ElemType e) {   //在顺序线性表L中第 i (1≤i≤L.length+1)个位置之前插入元素e,
...
Status GetElem(SqList L, int i, ElemType &e) { //e is where you're going to return the result.
    //Here is wating for you.
    if(i <= L.length && i>=1){
        e = L.elem[i-1];
        return OK;
